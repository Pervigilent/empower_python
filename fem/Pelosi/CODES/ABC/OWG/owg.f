...
C
C  =====================================================================
C     Open Rectangular Waveguide-devices in H or E plane
...
C  =====================================================================
      ...
      parameter (
     &  MAXNODE      = 1250,     !Maximum number of nodes
     &  MAXELE       = 1460,     !Maximum number of elements
...
c     FEM Mesh and related variables
      ...
      integer ABCTYPE            !Type of ABC
                                 ...
                                 ! 2 = PMA
      integer Lpec,Lpmc,Labc     !Label of nodes on pec,pmc and abc
...
C     STARTUP      
     ...
     &      form='formatted',file=felename)
      ...
      read (1,*) r               !Extimeed center of radiation
      read (1,*) (Lp(i),i=1,np)  !Port Labels
      read (1,*) lpec,lpmc       !Labels on pec and pmc
      read (1,*) LRM             !Label of the radiating mouth nodes
...
C     Creates the vectors of the nodes on the ports
      LP(np+1)=LRM
      call ports(MAXNODE,MAXNXP,NLAB,XY,PO,np+1,nnode,dim,LP)
...
C     FEM Matrix building
        if (ABCTYPE.eq.1) then
          call BUILD_FEMa(MAXNODE,MAXNXE,MAXELE,MAXBAND,MAXNXP,
     &                    ELE,xy,NLAB,ELAB,nele,nnode,Lpec,Lpmc,PO,
     &                    Ndie,Material,Eps,Np+1,FEM,ku,kl,kd,
     &                    Plane,Vcoeff,vkt2,AR,BR,labc) 
        else
          call BUILD_FEMb(MAXNODE,MAXNXE,MAXELE,MAXBAND,MAXNXP,
     &                    ELE,xy,NLAB,ELAB,nele,nnode,Lpec,Lpmc,PO,
     &                    Ndie,Material,Eps,Np+1,FEM,ku,kl,kd,
     &                    Plane,Vcoeff,vkt2,LPMAX,LPMAY,LPMAXY,APMA) 
        endif
...
C     Solution
     ...
     &                IC,ID,NNODE,nport,npt,Nmode,V1,V2,1)
...
C     STORE RESULTS
     ...
     &                             atan2(aimag(v1(j)),real(v1(j)))
         ...
         if (flgfld.eq.1) then
           ...
           write(fieldfile,'(a3,I3.3,a4)'),"frm",k,".dat"
           open (unit=12,status='unknown',
     &           form='formatted',file=fieldfile)
           do j=1,PO(0,np+1) 
             write (12,*) real(V2(PO(j,np+1))),aimag(V2(PO(j,np+1))),
     &             abs(V2(PO(j,np+1))),
     &             atan2(aimag(V2(PO(j,np+1))),real(V2(PO(j,np+1))))
           enddo
           close(12)
         endif
